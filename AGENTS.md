...
- Harness de teste no device (MIUI bloqueia `adb input`): `adb shell setprop debug.gamenative.input "key:110"|"stick:x:y"|"hat:x:y"`. **PS = `KEYCODE_BUTTON_MODE` = 110**, não 188 (188 é `BUTTON_1`). Protocolo em `app/src/main/java/app/gamenative/ui/component/DebugGamepadInput.kt`. Verbos dos upgrades 2026-08-14: `touch:x:y`/`touchdown`/`touchup`/`touchtap` (U2 — exercita o gate de ghost input antes do consume) e `gyro:x:y:z` (U1 — injeta direto no `hub.onSensorSample`).
- Upgrades do intuito (spec 2026-08-14-gamepad-intuito-validacao-upgrades, specs U6/U2/U1/U3-U4/U5/U7): touchpad→mouse lê NO PONTO do gate do MainActivity (V7 — o gate continua consumindo; `GamepadTouchpadForwarder` + `TouchpadProcessor` puro); gyro via `GamepadSensorSource` (API 31+, `SENSOR_DELAY_GAME`, unregister em pause/exit — V3), `GyroProcessor` puro com recenter na borda de ativação, MOUSE via `xServerMouseSink` / CAMERA via `gyroCameraSink` → `applyCameraGyro`; camadas completas (`LayerResolver` puro, triggers HOLD/TOGGLE/DOUBLE_TAP, remap efetivo DEFAULT+ativa no hub e no `PhysicalControllerHandler` — gate ON; binding explícito vence, senão byte-identical); rumble por device (`GamepadHaptics.vibrateDevice`, API 16/31); bateria/capacidades por device (coleta no hotplug, UI na seção Gamepad). Store preserva chaves desconhecidas no save (V1 — nunca regravar mapa perdendo extras).
